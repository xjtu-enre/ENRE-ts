    ...
    case 100: {
      for (const ent of raw['entities']) {
        if (ent['type'] === 'File') {
          ...
          // Unmatched
            ...
            logger.warn(`Unmapped type understand/ts/entity/${type}`);
            continue;
          }
      ...
      for (const rel of raw['relations']) {
        ...
        if (/Import/.test(type) || /Require/.test(type)) {
          ...
          for (const iRel of raw['relations']) {
            ...
            if (
              ...
              extra.alias = (aliasEnt.name as ENREName<any>).string;
              break;
            }
          }
        }
        ...
        // Unmapped
          ...
          logger.warn(`Unmapped type understand/ts/relation/${type}`);
          continue;
        }
